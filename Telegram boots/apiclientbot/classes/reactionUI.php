...
class reactionUI extends PDO
{
	...
	public function __construct($data, $file = 'my_setting.ini')
    {
        ...
        $this->url='https://api.telegram.org/botxxxxxxxxxxxxxxxxxxxxx/sendMessage';
        $this->urldoc='https://api.telegram.org/botxxxxxxxxxxxxxxxxxxxxxxxxx/sendDocument';
        ...
        switch ($xman)
        {
            case 0:
                switch ($this->ms)
                {
                    case "Да":
                        $this->updateAge18();
                        $this->sendMessage($this->chatID, "Я мужчина", $this->menu1);
                        break;
                    case "Выход":
    ...
    public function sendFile($IDName, $sort) {
        $file_url=$this->securityStrip("https://xxxxxxxx/api/exel.php?user=$IDName&sort=$sort");
    ...
    public function sendMail()
    {
        ...
        try {
            ...
            $mail->Host = 'mail.xxxxxxxxx.kz';                    // Set the SMTP server to send through
            $mail->SMTPAuth = true;                                   // Enable SMTP authentication
            $mail->Username = 'info@xxxxxxxxxx.kz';                     // SMTP username
            $mail->Password = 'Pf679i^r';                               // SMTP password
            ...
            $mail->setFrom('info@xxxxxxxx.kz', $this->userArray["sekret"]." - запрос на тест от ".date("d-m-Y"));
            $mail->addAddress('test@xxxxxxxxxx.planfix.ru', 'sMM');     // Add a recipient
            // Content
